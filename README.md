...
Колесниченко Ксения, Мерзликин Артем, Плотников Кирилл, Тарасов Дмитрий, Шадрин Ярослав

## Описание

Из музыкальной базы данных Musicbrainz собраны названия российских музыкальных исполнителей. 
Затем, с помощью Spotify API, собраны данные об этих артистах: количество их подписчиков, индекс популярности, жанры, а также их лучшие композиции. 
О каждом треке собрана следующая информация: название, дата релиза, длительность, индекс популярности, особенности аудио и другие.

Такой датасет позволит создавать B2B и B2C продукты в музыкальной сфере. 
Например, системы рекомендаций для промоутеров, помогающие с выбором исполнителей, концерт которых наиболее востребован в данный момент.
Или системы помощи молодым исполнителям в написании композиций, которые с большей вероятностью станут популярными.
...
│   ├── artists.csv  # данные об артистах
│   ├── intermediate  # промежуточные данные
│   │   ├── artist_top_tracks.csv
│   │   └── musicbrainz_artists.csv
│   └── tracks.csv  # данные о треках
├── README.md
└── scrapers  # директория со скриптами для получения данных
    ├── __init__.py
    ├── musicbrainz_get_artists.py
    ├── requirements.txt
    ├── spotify_fetch_artist_info.py
    ├── spotify_fetch_artist_top_tracks.py
    ├── spotify_fetch_top_tracks_features.py
    └── st_utils.py
```
